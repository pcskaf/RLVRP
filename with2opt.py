...
@reference: Nazari, M., Oroojlooy, A., Snyder, L. V., and Taka ÃŒÂc, M. Reinforcement learning for solving the vehiclerouting problem.
"""
...
def findNextCost(previous, next, coords):
    return coords[previous][next]

def calculate_total_cost(cost_mat, route):
    total = 0
    for i in range(0, (len(route)-1)):
        total = sum(total, findNextCost(route[i], route[i+1], cost_mat))
    ...
    while improved:
        ...
        for i in range(1, len(route) - 2):
            for j in range(i + 1, len(route)):
                ...
                # print(new_route)
                calculate_total_cost(cost_mat, new_route)
                if cost(cost_mat, new_route) < cost(cost_mat, best):
                # if calculate_total_cost(cost_mat, new_route) < calculate_total_cost(cost_mat, best):
                    best = new_route
                    improved = True
                    route = best
    return best
    
def getRoute(data, actions):
    route=[]
    ...
    # print(actions[1][0][0])
    # print(actions[1][0][1])
    #print(actions[0][0], actions[0][1])
    # print(data)
    # print(np.array_equal(data[0],actions[1][0]))
    # print(np.array_equal(data[1],actions[1][0]))
    # print(np.array_equal(data[2],actions[1][0]))
    # print(np.array_equal(data[3],actions[1][0]))
    # print(np.array_equal(data[4],actions[0][0]))
    # print(np.array_equal(data[5],actions[0][0]))
    # print(np.array_equal(data[6],actions[0][0]))
    # print(np.array_equal(data[7],actions[0][0]))
    # print(np.array_equal(data[8],actions[0][0]))
    # print(np.array_equal(data[9],actions[0][0]))
    # print(np.array_equal(data[10],actions[0][0]))
    # print(np.array_equal(data[11],actions[0][0]))
    # print(np.array_equal(data[12],actions[0][0]))
    # print(np.array_equal(data[13],actions[0][0]))
    # print(np.array_equal(data[14],actions[0][0]))
    # print(np.array_equal(data[15],actions[0][0]))
    # print(np.array_equal(data[16],actions[0][0]))
    # print(np.array_equal(data[17],actions[0][0]))
    # print(np.array_equal(data[18],actions[0][0]))
    # print(np.array_equal(data[19],actions[0][0]))
    # print(np.array_equal(data[20],actions[0][0]))
    # print(np.array_equal(data[21],actions[0][0]))
    for i in range(0,len(actions)-1):
        #print(i)
        for j in range(0,len(data)-1):
            if np.array_equal(data[j],actions[i][0]):
                if np.array_equal(data[j],actions[i][0]):
                    route.append(j)
                    break
    return route
...
def getCostMat(data):
    ...
    for i in range(len(data)):
        for j in range(len(data)):
            hav.append(getdistance(data[i], data[j]))

    cost_mat = np.reshape(hav, (len(data), len(data)))  # reshaping to 10 x 10 matrix
    return cost_mat
...
def create_VRP_dataset(
    ...
    #data_dir="/home/thesisuser1/Documents/RLVRP/data/C14-101.txt"
    # cteate/load data
    #if os.path.exists(fname):
    if os.path.exists(data_dir):
        print('Loading dataset for {}...'.format(data_dir))
        #data = np.loadtxt(fname,delimiter=' ')
        data = np.loadtxt(data_dir,delimiter=' ')
        #print(len(data_dir))
        
        #filepath = data_dir.split("-")[len(data_dir.split("-"))-1]
        #data = data.reshape(-1, n_nodes,3)
        #nodenum=int(filepath.split(".")[0])
        #data = data.reshape(-1, nodenum,3)
        data = data.reshape(-1, n_nodes,3)
    #else:
...
class Env(object):
    def __init__(self,
        ...
        #self.n_nodes = nodenum
        self.n_cust = args['n_cust']
        #self.n_cust = nodenum-1
...
class RLAgent(object):
    ...
    def evaluate_batch(self,eval_type='greedy'):
        ...
        for i in range(len(data[0])):
            data2.append([data[0][i][0], data[0][i][1]])
        #print(data2)
        route=[]
        route.append(getRoute(data2,actions))
        
        cost_mat=getCostMat(data2)
        # print(cost(cost_mat, route[0]))
        #print(cost_mat)
        newcost=two_opt(cost_mat,route[0])
        # print(newcost)
        #print(len(R))
        end_time = time.time() - start_time + end_time
        # self.prt.print_out('Average of {} in batch-mode: {} -- std {} -- time {} s'.format(eval_type,\
        #     np.mean(R),np.sqrt(np.var(R)),end_time))        
        # self.prt.print_out('Average_2opt of {} in batch-mode: {} -- std {} -- time {} s'.format(eval_type,\
        #     calculate_total_cost(cost_mat, newcost),np.sqrt(np.var(R)),end_time))        
        self.prt.print_out('Average_2opt of {} in batch-mode: {} -- std {} -- time {} s'.format(eval_type,\
            cost(cost_mat, newcost),np.sqrt(np.var(R)),end_time))        
...
vrpc1 = TaskVRP(task_name = 'vrp',
			  ...
			  capacity=160,
			  demand_max=50)
task_lst['vrpc1'] = vrpc1

# VRPC2
vrpc2 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=76,
			  n_cust = 75,
			  decode_len=100,
			  capacity=140,
			  demand_max=50)
task_lst['vrpc2'] = vrpc2

# VRPC3
vrpc3 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=101,
			  n_cust = 100,
			  decode_len=140,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc3'] = vrpc3

# VRPC4
vrpc4 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=151,
			  n_cust = 150,
			  decode_len=210,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc4'] = vrpc4

# VRPC5
vrpc5 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=200,
			  n_cust = 199,
			  decode_len=280,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc5'] = vrpc5

# VRPC6
vrpc6 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=51,
			  n_cust = 50,
			  decode_len=70,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc6'] = vrpc6

# VRPC7
vrpc7 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=76,
			  n_cust = 75,
			  decode_len=100,
			  capacity=140,
			  demand_max=50)
task_lst['vrpc7'] = vrpc7

# VRPC8
vrpc8 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=101,
			  n_cust = 100,
			  decode_len=140,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc8'] = vrpc8

# VRPC9
vrpc9 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=151,
			  n_cust = 150,
			  decode_len=210,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc9'] = vrpc9

# VRPC10
vrpc10 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=200,
			  n_cust = 199,
			  decode_len=280,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc10'] = vrpc10

# VRPC11
vrpc11 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=121,
			  n_cust = 120,
			  decode_len=168,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc11'] = vrpc11

# VRPC12
vrpc12 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=101,
			  n_cust = 100,
			  decode_len=140,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc12'] = vrpc12

# VRPC13
vrpc13 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=121,
			  n_cust = 120,
			  decode_len=168,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc13'] = vrpc13

# VRPC14
vrpc14 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=101,
			  n_cust = 100,
			  decode_len=140,
			  capacity=200,
			  demand_max=50)
task_lst['vrpc14'] = vrpc14

# VRPM101
vrpm101 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=101,
			  n_cust = 100,
			  decode_len=140,
			  capacity=200,
			  demand_max=50)
task_lst['vrpm101'] = vrpm101

# VRPM121
vrpm121 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=121,
			  n_cust = 120,
			  decode_len=168,
			  capacity=200,
			  demand_max=50)
task_lst['vrpm121'] = vrpm121

# VRPM151
vrpm151 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=151,
			  n_cust = 150,
			  decode_len=210,
			  capacity=200,
			  demand_max=50)
task_lst['vrpm151'] = vrpm151

# VRPM200
vrpm200 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=200,
			  n_cust = 199,
			  decode_len=280,
			  capacity=200,
			  demand_max=50)
task_lst['vrpm200'] = vrpm200

# VRPe22
vrpe22 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=22,
			  n_cust = 21,
			  decode_len=30,
			  capacity=6000,
			  demand_max=2500)
task_lst['vrpe22'] = vrpe22

# VRPe23
vrpe23 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=23,
			  n_cust = 22,
			  decode_len=30,
			  capacity=4500,
			  demand_max=4100)
task_lst['vrpe23'] = vrpe23

# VRPe30
vrpe30 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=30,
			  n_cust = 29,
			  decode_len=45,
			  capacity=4500,
			  demand_max=2500)
task_lst['vrpe30'] = vrpe30

# VRPe51
vrpe51 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=51,
			  n_cust = 50,
			  decode_len=70,
			  capacity=160,
			  demand_max=50)
task_lst['vrpe51'] = vrpe51

# VRPe76_7
vrpe76_7 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=76,
			  n_cust = 75,
			  decode_len=100,
			  capacity=220,
			  demand_max=50)
task_lst['vrpe76_7'] = vrpe76_7

# VRPe76_8
vrpe76_8 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=76,
			  n_cust = 75,
			  decode_len=100,
			  capacity=180,
			  demand_max=50)
task_lst['vrpe76_8'] = vrpe76_8

# VRPe76_10
vrpe76_10 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=76,
			  n_cust = 75,
			  decode_len=100,
			  capacity=140,
			  demand_max=50)
task_lst['vrpe76_10'] = vrpe76_10

# VRPe76_14
vrpe76_14 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=76,
			  n_cust = 75,
			  decode_len=100,
			  capacity=100,
			  demand_max=50)
task_lst['vrpe76_14'] = vrpe76_14

# VRPe101_8
vrpe101_8 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=101,
			  n_cust = 100,
			  decode_len=140,
			  capacity=200,
			  demand_max=50)
task_lst['vrpe101_8'] = vrpe101_8

# VRPe101_14
vrpe101_14 = TaskVRP(task_name = 'vrp',
			  input_dim=3,
			  n_nodes=101,
			  n_cust = 100,
			  decode_len=140,
			  capacity=112,
			  demand_max=50)
task_lst['vrpe101_14'] = vrpe101_14
